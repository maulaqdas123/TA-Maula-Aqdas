...
class Bus
{
    ...
    public $penumpangAktif; // TIPE DATA: Array asosiatif
    public $imageUrl;
    ...
    public function tapIn($nim, $dariHalte, $keHalte)
    {
...
foreach ($busList as $bus) {
    ...
    if (!isset($_SESSION[$sessionKey])) {
        if (strpos($bus->nama, 'Dipyo-1') !== false) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bus_id_action'])) {
    ...
    foreach ($busList as $bus) {
        if ($bus->id === $busId) {
            if ($actionType === 'tap_in') { $actionMessage = $bus->tapIn($userId, $_POST['halte_dari'], $_POST['halte_ke']); }
...
.sidebar{width:80px;background-color:var(--card-dark);padding:1.5rem 0;display:flex;flex-direction:column;align-items:center;box-shadow:4px 0 10px rgba(0,0,0,0.5);border-right:1px solid #333;}
...
<div class="main-content">
    ...
    <div class="bus-carousel-container">
        ...
        foreach ($busList as $bus) {
            ...
            echo "<span class='details-button'>Detail & Tap In/Out</span>";
            echo "</div>";
            echo "</a>";
        }
        ?>
...
<div class="modal-overlay" id="tapModal">
    <div class="modal-content">
        <?php if ($tappedBus): ?>
            ...
            $userTrip = $isUserTappedIn ? $tappedBus->penumpangAktif[$currentUserId] : null;
            // PENGKONDISIAN: Logika Tap In/Out
            if ($tappedBus->getKursiTersedia() <= 0 && !$isUserTappedIn) {
                // GUI: Bus Penuh
